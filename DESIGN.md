---
name: waverr
description: A directory index of one program — near-black ground, hairline column rules, and one white handheld device.
colors:
  ink: "#0b0b0b"
  ink-2: "#121212"
  rule: "#262626"
...
typography:
  display:
    ...
    fontSize: "clamp(2.3rem, 6.2vw, 4.6rem)"
    fontWeight: 500
    lineHeight: 1.02
...
- Continuous full-height vertical column rules, painted on the region, not per-listing
...
### Tertiary — LCD Phosphor
Four green-tinted neutrals that exist only inside the depicted device and never appear on the page around it.
- **LCD Ground** (`{colors.lcd-ground}`): The screen's dark field.
- **LCD Text** (`{colors.lcd-text}`): The selected track row's fill and the screen's base text colour.
- **LCD Row** (`{colors.lcd-dim}`): Unselected track rows.
- **LCD Chrome** (`{colors.lcd-chrome}`): The status bar, the battery cell outline, the visualizer bars.
...
**The Phosphor Containment Rule.** The four green-tinted LCD neutrals live inside the device and nowhere else. Tinting page chrome with them turns a depicted screen into a theme.
...
### Hierarchy
- **Display** (500, `clamp(2.3rem, 6.2vw, 4.6rem)`, 1.02, `-0.045em`): The page title only, set as a two-line listing entry — "Index of" over a dimmed path.
...
**Column rules.** Two vertical hairlines paint as background gradients on `.region` and `.ruled` rather than as borders on a listing, positioned from the container's centre so they land exactly on the two internal gutters. Because they paint on the region, they run its full height regardless of how much content sits inside, which is what makes the index a world rather than a costume worn by three sections.
...
**The Continuous Rule Rule.** Column rules belong to the region, not the listing. A rule that starts and stops with a table is a table border, not a world.
...
### Shadow Vocabulary
- **Device cast** (`box-shadow: 0 34px 70px rgba(0,0,0,.66), 0 8px 22px rgba(0,0,0,.42)`): Two stacked outer shadows — a wide ambient drop and a tighter contact shadow — on the handheld device only.
- **LCD recess** (`box-shadow: 0 2px 8px rgba(0,0,0,.45) inset`): A single inset shadow sinking the screen into the chassis.

The device chassis is a flat ivory fill. The imitation-plastic gradients and inset bevels that first shipped on it were removed at review; the object reads as physical from its silhouette, its value against the ground, and one real cast shadow.
...
**The No Fake Material Rule.** The device is flat fills. No plastic gradients, no bevels, no simulated highlights. Physicality comes from shape, value and cast shadow.
...
The one physical object. A `min(318px, 100%)` ivory chassis at `24px` radius with the two-shadow cast, containing a `5px`-radius LCD in phosphor neutrals — status bar, battery cell, four-row track list with one inverted selected row, and a 28-bar visualizer — above a `186px` circular click wheel with a MENU label, three SVG transport marks and a concentric `70px` centre button. The visualizer bars animate on an `820ms` alternating ease-in-out keyframe; the script randomizes each bar's duration (460–1080ms) and negative delay so the block reads as audio rather than as a loading indicator, and skips seeding entirely under reduced motion. At `≤680px` the chassis caps at `290px` and the wheel drops to `162px`.
...
- **Do** paint column rules on the region (`.region, .ruled`) so they run its full height.
